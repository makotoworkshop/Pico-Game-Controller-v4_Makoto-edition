...
void init() {
  ...
  // RGB Mode Switching : ici pour changer de mode en turbocharger en appuiyant une touche à l'allumage, sinon reste en mode caroussel
...
//###################################################################################
#define Bouton_SW2 16  // GPIO du bouton SW2
#define Bouton_SW3 17  // GPIO du bouton SW3
int SW2_state = 0;
int SW3_state = 0;

int selection_mode_decoration() {
  gpio_init(Bouton_SW2);
  gpio_set_dir(Bouton_SW2, GPIO_IN);
  gpio_pull_up(Bouton_SW2);  // Active la résistance de pull-up interne
  
  gpio_init(Bouton_SW3);
  gpio_set_dir(Bouton_SW3, GPIO_IN);
  gpio_pull_up(Bouton_SW3);  // Active la résistance de pull-up interne

  if (gpio_get(Bouton_SW2) == 0) {  // 0 = bouton pressé (pull-up actif)
    SW2_state ++;
    sleep_ms(200);  // Anti-rebond humain
    switch(SW2_state) {
      case 0:
        break;
      case 1:
        ws2812b_mode = &ws2812b_color_64;  // VERT jaune
        break;
      case 2:
        ws2812b_mode = &ws2812b_color_128;  // JAUNE
        break;
      case 3:
        ws2812b_mode = &ws2812b_color_192;  // ORANGE
       break;   
      case 4:
        ws2812b_mode = &ws2812b_color_256;  // ROUGE
        break;  
      case 5:
        ws2812b_mode = &ws2812b_color_320;  // ROSE
        break;   
      case 6:
        ws2812b_mode = &ws2812b_color_384;  // VIOLET
        break;   
      case 7:
        ws2812b_mode = &ws2812b_color_448;  // BLANC
        break;   
      case 8:
        ws2812b_mode = &ws2812b_color_512;  // BLEU
        break;   
      case 9:
        ws2812b_mode = &ws2812b_color_576;  // BLEU clair
        break;   
      case 10:
        ws2812b_mode = &ws2812b_color_640;  // BLEU clair clair
       break;  
      case 11:
        ws2812b_mode = &ws2812b_color_704;  // BLEU VERT
       break;         
      case 12:
        ws2812b_mode = &ws2812b_color_768;  // VERT
                SW2_state = 0;
       break;   
      default:
        // code block
  ...
  if (gpio_get(Bouton_SW3) == 0) {  // 0 = bouton pressé (pull-up actif)
    SW3_state ++;
    sleep_ms(200);  // Anti-rebond humain
